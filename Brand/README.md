...
TUFF Violet is `#6F4DFF`. `TUFFLogo.svg` is the transparent single-color mark for documentation and other flat uses.

`TUFF.icon` is the editable 1024 by 1024 Icon Composer source. Its graphite canvas and three separate TUFF Violet facet groups use matching material settings to produce the default, dark, clear, and tinted system renditions. Open it in Icon Composer to tune material settings without baking blur, shadows, or highlights into the SVG artwork.
