...
public struct AppPersistedChatTurn: Codable, Equatable, Sendable, Identifiable {
    ...
    /// Attached files, stored with their extracted text: a saved chat that is
    /// continued has to send the same document text the first answer saw.
    ...
    /// carried documents or a model name — still loads rather than failing the
    /// whole file and putting the store into read-only mode.
    public init(from decoder: Decoder) throws {
        ...
        attachments = try container.decodeIfPresent(
            [AppConversationAttachment].self, forKey: .attachments) ?? []
    ...
    /// from relative paths, so it fills them in.
