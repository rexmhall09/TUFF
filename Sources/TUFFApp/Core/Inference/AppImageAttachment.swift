...
public struct AppImageAttachment: Sendable, Equatable, Codable {
    public let id: UUID
...
public struct AppImageAttachmentStore: Sendable {
    public static let rootName = "TUFF-Attachments"
    /// The staging root written by v2 and earlier. Nothing stages there any
    /// more, but the sweep still visits it: renaming the root without this left
    /// every full-size copy an older build had staged in the temporary
    /// directory with nothing left that would ever delete it.
    static let legacyRootName = "TurboFieldfare-Attachments"
    ...
    static var legacyRoot: URL {
        FileManager.default.temporaryDirectory
            .appendingPathComponent(legacyRootName, isDirectory: true)
    }
