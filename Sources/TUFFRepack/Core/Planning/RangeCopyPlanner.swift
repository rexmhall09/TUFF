...
public enum RangeCopyPlanner {
    static func plan(
        ...
        //
        // The domain keeps its pre-v3 spelling deliberately. It is hashed into
        // the fingerprint a saved checkpoint carries, and a resumed download
        // whose fingerprint no longer matches is thrown away — renaming it for
        // tidiness would restart every partial install in the field.
    ...
    /// The default domain keeps its pre-v3 spelling: it is hashed into the
    /// fingerprint stored in a download checkpoint, and a mismatch discards the
    /// saved progress. See the vision planner above.
