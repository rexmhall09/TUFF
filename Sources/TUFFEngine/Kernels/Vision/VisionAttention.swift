...
public final class VisionAttention {
    ...
    public init(context: MetalContext,
        ...
        if headDimension == 64 {
            ...
            usesFusedPaddedLayout = false
            softmaxPhase = .serial
        } else if environment["TUFF_VISION_ATTENTION_PAD80"] == "1" {
            variant = .mlxPadded80
            queryTile = 8
            ...
            usesFusedPaddedLayout = false
            softmaxPhase = .serial
        } else if environment["TUFF_VISION_ATTENTION_MPP"] == "1" {
            ...
            let fused = allowFusedPaddedLayout
                && environment["TUFF_VISION_PAD_ROUNDTRIP"] != "1"
            ...
            if environment["TUFF_VISION_ATTENTION_PARALLEL_SOFTMAX"] == "1" {
        ...
        } else if environment["TUFF_VISION_ATTENTION_Q8"] == "1" {
            variant = .native72
            queryTile = 8
            pipeline = try context.pipeline("vision_attention_online_72")
            nativeQueryTile = 8
