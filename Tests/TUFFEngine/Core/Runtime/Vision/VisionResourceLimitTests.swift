...
@Suite struct VisionResourceLimitTests {
    ...
    @Test func defaultKernelSelectionKeepsPromotedAndBaselinePathsDistinct() {
        ...
        #expect(promoted["TUFF_VISION_ATTENTION_MPP"] == "1")
        #expect(promoted["TUFF_VISION_REGISTER_GEMM"] == "1")

        let baseline = VisionRuntime.resolvedKernelEnvironment(
            ["TUFF_VISION_BASELINE_KERNELS": "1"])
        #expect(baseline["TUFF_VISION_ATTENTION_MPP"] == nil)
        #expect(baseline["TUFF_VISION_REGISTER_GEMM"] == nil)

        let explicit = VisionRuntime.resolvedKernelEnvironment([
            "TUFF_VISION_ATTENTION_Q8": "1",
            "TUFF_VISION_REGISTER_ATTENTION": "1",
        ])
        #expect(explicit["TUFF_VISION_ATTENTION_MPP"] == nil)
        #expect(explicit["TUFF_VISION_REGISTER_GEMM"] == nil)
    }
    ...
    static var qwenOptInModelURL: URL? {
        guard ProcessInfo.processInfo.environment[
            "TUFF_VALIDATE_QWEN_VISION"] == "1" else { return nil }
        let manager = FileManager.default
    ...
    @Test(.enabled(if: Self.qwenOptInModelURL != nil && Self.supportsVisionRuntime,
                   "set TUFF_VALIDATE_QWEN_VISION=1 with the local Qwen pack installed"))
